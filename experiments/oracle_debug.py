...
def parse_args():
    ...
    parser.add_argument('--bad_policy', type=str, default='maddpg_mmmaddpg',
    ...
    parser.add_argument('--lemol_save_path', type=str,
                        default='./tmp/{}_vs_{}/OM{}/{}.npz', help='path to save experience for om training')
...
def train(arglist):
    ...
    with U.single_threaded_session() as sess:
        ...
        vary_good = False
        vary_bad = False
        if len(good_policies) > 1 or len(bad_policies) > 1:
            if len(good_policies) > 1:
                assert len(
                    bad_policies) == 1, 'Training should only vary good OR bad agents.'
                print('Training with varied good agents and constant {} bad agent.'.format(
                    bad_policies[0].name))
                vary_good = True
            else:
                assert len(
                    good_policies) == 1, 'Training should only vary good OR bad agents.'
                vary_bad = True
                print('Training with varied bad agents and constant {} good agent.'.format(
                    good_policies[0].name))

        lemol_agent_index = None
        using_lemol = 'lemol' in arglist.good_policy.lower(
        ) or 'lemol' in arglist.bad_policy.lower()
        ...
        if using_lemol:
            lemol_ip_acc_placeholder = tf.placeholder(
                tf.float32, shape=(), name='om_xent_logging_ph')
            lemol_om_ip_acc_summary = tf.summary.scalar(
                'in_play_lemol_om_prediction_accuracy', lemol_ip_acc_placeholder)
            lemol_ip_xent_placeholder = tf.placeholder(
                tf.float32, shape=(), name='om_xent_logging_ph')
            lemol_om_ip_xent_summary = tf.summary.scalar(
                'in_play_lemol_om_prediction_xent', lemol_ip_xent_placeholder)
            lemol_om_in_play_summary = tf.summary.merge(
                [lemol_om_ip_acc_summary, lemol_om_ip_xent_summary]
            )

        # ratios_of_feed_lemol_true_action = [0.0, 0.5, 0.6, 0.7, 0.8, 1.0]
        ratios_of_feed_lemol_true_action = [1.0, 0.0, 0.5]

        # Initialize
        U.initialize()
        outer_iterations = 1
        traj_per_iter = len(ratios_of_feed_lemol_true_action)
        for outer_iter in range(outer_iterations):
            for k in range(traj_per_iter):
                for j in range(max(len(good_policies), len(bad_policies))):
                    ...
                    np.random.seed(71)
                    random.seed(71)
                    ratio = ratios_of_feed_lemol_true_action[k]
                    if vary_bad:
                    ...
                    for i, trainer in enumerate(current_pairing):
                        if isinstance(trainer, LeMOLAgentTrainer):
                            lemol_agent_index = i
                    ...
                    if using_lemol:
                        current_pairing[lemol_agent_index].set_save_dir(
                            j, outer_iter, k, arglist)
                        ...
                        while True:
                                ...
                                if arglist.feed_lemol_true_action:
                                    if np.random.uniform() <= ratio:
                                        if arglist.continuous_actions:
                                            ...
                                            om_pred = np.zeros_like(action_n[1 - lemol_agent_index])
                                            om_pred[np.argmax(action_n[1 - lemol_agent_index])] = 1.0
                                    else:
                                        if arglist.continuous_actions:
                                            om_pred = np.random.dirichlet((1.1,) * len(action_n[1 - lemol_agent_index]))
                                        else:
                                            om_pred = np.zeros_like(action_n[1 - lemol_agent_index])
                                            om_pred[np.random.choice(env.action_space[1-lemol_agent_index].n)] = 1.0
                                else:
                                    lemol_agent = current_pairing[lemol_agent_index]
                                    if len(lemol_agent.replay_buffer) > lemol_agent.max_replay_buffer_len:
                                        om_pred, h, c = current_pairing[lemol_agent_index].om_step(
                                            lstm_input, obs_n[lemol_agent_index], h, c)
                                    else:
                                        om_pred = np.random.dirichlet(
                                            (1.25,) * action_n[1-lemol_agent_index].shape[-1])
                                action_n[lemol_agent_index] = current_pairing[lemol_agent_index].action(
                                    obs_n[lemol_agent_index], om_pred)
                            else:
                                action_n = []
                            ...
                            for i, agent in enumerate(current_pairing):
                                if isinstance(agent, LeMOLAgentTrainer):
                                    agent.experience(
                                        obs_n[i], action_n[i], rew_n[i], new_obs_n[i], done_n[i], terminal, om_pred, h, c, action_n[1-i], None, None)
                                    if arglist.log_om_influence and train_step % arglist.log_om_influence_freq == 0:
                                        agent.log_om_influence(
                                            obs_n[i], np.squeeze(om_pred), general_summary_writer, train_step)
                                else:
                                    agent.experience(
                            ...
                            if not arglist.test:
                                ...
                                for i, agent in enumerate(current_pairing):
                                    loss = agent.update(
                                        current_pairing, train_step, lemol_agent_index)
                                    if loss is not None:
                            ...
                            if summary_values:
                                ...
                                file_writer.add_summary(
                                    s, train_step)
                                if (using_lemol and
                                        train_step % arglist.lemol_in_play_perf_log_freq == 0):
                                    acc_value, xent_value = current_pairing[lemol_agent_index]\
                                        .replay_buffer.get_in_om_performance_metrics()
                                    if acc_value is not None and xent_value is not None:
                                        s = sess.run(lemol_om_in_play_summary,
                                                     {
                                                         lemol_ip_acc_placeholder: acc_value,
                                                         lemol_ip_xent_placeholder: xent_value
                                                     })
                                        general_summary_writer.add_summary(
                                            s, train_step)
            ...
            if using_lemol and not arglist.feed_lemol_true_action:
                ...
                for _ in range(arglist.om_training_iters):
                    current_pairing[lemol_agent_index].train_opponent_model(
